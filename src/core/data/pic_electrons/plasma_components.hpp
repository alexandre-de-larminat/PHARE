...
namespace PHARE::core
{
    ...
    public:
        ...
        using tensorfield_type    = typename ParticlePopulation::tensorfield_type;
        using particle_array_type = typename ParticlePopulation::particle_array_type;
        ...
        virtual std::vector<ParticlePopulation> const& getPopulations() const {return populations_;}

        virtual std::size nbrPopulations() const { return getPopulations().size(); }
        ...
        void computeBulkVelocity()
        {
            ...
            velocity().zero();
            auto& vx = velocity().getComponent(Component::X);
            auto& vy = velocity().getComponent(Component::Y);
            auto& vz = velocity().getComponent(Component::Z);
