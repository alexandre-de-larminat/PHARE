...
TEST(APythonDataProvider, providesAValidTree)
{
    ...
    auto& pop1ParticleInitializer = pop1["particle_initializer"];
    auto pop1ParticleInitializerName = pop1ParticleInitializer["name"].to<std::string>();
    auto pop1density                 = pop1ParticleInitializer["density"].to<InitFunction<1>>();
    auto bulk1x             = pop1ParticleInitializer["bulk_velocity_x"].to<InitFunction<1>>();
    auto bulk1y             = pop1ParticleInitializer["bulk_velocity_y"].to<InitFunction<1>>();
    auto bulk1z             = pop1ParticleInitializer["bulk_velocity_z"].to<InitFunction<1>>();
    auto vth1x              = pop1ParticleInitializer["thermal_velocity_x"].to<InitFunction<1>>();
    auto vth1y              = pop1ParticleInitializer["thermal_velocity_y"].to<InitFunction<1>>();
    auto vth1z              = pop1ParticleInitializer["thermal_velocity_z"].to<InitFunction<1>>();
    auto pop1NbrPartPerCell = pop1ParticleInitializer["nbr_part_per_cell"].to<int>();
    auto pop1Charge         = pop1ParticleInitializer["charge"].to<double>();
    auto pop1Basis          = pop1ParticleInitializer["basis"].to<std::string>();
    ...
    EXPECT_EQ(1, nbrPopulations);
    EXPECT_EQ("electrons", pop1Name);
    EXPECT_DOUBLE_EQ(.1, pop1Mass);
    EXPECT_EQ("maxwellian", pop1ParticleInitializerName);
    EXPECT_DOUBLE_EQ(2., (*pop1density(input_2))[0]);
    EXPECT_DOUBLE_EQ(1., (*bulk1x(input_2))[0]);
    EXPECT_DOUBLE_EQ(1., (*bulk1y(input_2))[0]);
    EXPECT_DOUBLE_EQ(1., (*bulk1z(input_2))[0]);
    EXPECT_DOUBLE_EQ(1., (*vth1x(input_2))[0]);
    EXPECT_DOUBLE_EQ(1., (*vth1y(input_2))[0]);
    EXPECT_DOUBLE_EQ(1., (*vth1z(input_2))[0]);
    EXPECT_EQ(100, pop1NbrPartPerCell);
    EXPECT_DOUBLE_EQ(-1., pop1Charge);
    EXPECT_EQ("cartesian", pop1Basis);
